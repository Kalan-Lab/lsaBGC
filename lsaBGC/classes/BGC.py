...
class BGC:
	...
	def refineGenbank(self, refined_genbank_file, first_bg, second_bg):
		...
		try:
			...
			with open(self.bgc_genbank) as ogbk:
				try:
					assert(len(SeqIO.parse(ogbk, 'genbank')) == 1)
				...
				for rec in SeqIO.parse(ogbk, 'genbank'):
					...
					new_seq_object = Seq(filtered_seq, IUPAC.unambiguous_dna)

					# Create a record
					updated_record = SeqRecord(sequence_object,
