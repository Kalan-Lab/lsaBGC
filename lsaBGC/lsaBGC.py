		...
		for i, bgc1 in enumerate(bgc_cogs):
			...
			for j, bgc2 in enumerate(bgc_cogs):
				if i < j:
					...
					overlap_metric = float(len(bgc1_cogs.intersection(bgc2_cogs))) / float(
						len(bgc1_cogs.union(bgc2_cogs)))
		...
		for i, line in enumerate(ofm):
			...
			if flag_in_bgc:
				...
				if calc_prop_multicopy:
					cog_median_gene_counts[cog] = float(sum([1 for x in gene_counts if x > 1])) / sum(
