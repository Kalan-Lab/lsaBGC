...
if [[ $# -eq 0 || $1 == "-h" || $1 == "--help" || $1 == "-v" || $1 == "--version" ]]; then
	docker pull raufs/lsabgc_wosp:latest
	docker run \
	--detach=false \
	--rm \
	--user=$(id -u):$(id -g) \
	raufs/lsabgc_wosp:latest \
  ...
  # run workflow
  docker pull raufs/lsabgc_wosp:latest
  docker run ${DOCKER_VOLUME_ARGS} --detach=false --rm --user=$(id -u):$(id -g) raufs/lsabgc_wosp:latest ${EASY_ARGS}
